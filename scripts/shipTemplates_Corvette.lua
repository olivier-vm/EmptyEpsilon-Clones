...
template:setDescription([[The Atlantis X23 is the smallest model of destroyer, and its combination of frigate-like size and corvette-like power makes it an excellent escort ship when defending larger ships against multiple smaller enemies. Because the Atlantis X23 is fitted with a WARP drive, it can also serve as an intersystem patrol craft.]])
...
template = ShipTemplate():setName("WARP Carrier"):setClass("Corvette", "Freighter"):setModel("transport_4_2")
template:setDescription([[The WARP Carrier is a specialized Freigher. It does not carry any cargo, as it's cargo bay is taken up by a specialized WARP drive and the energy storage required to run this WARP drive.
