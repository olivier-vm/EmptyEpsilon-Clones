...
function mainMenu()
    ...
    if comms_target:areEnemiesInRange(5000) then
        setCommsMessage("Nous sommes attaques! Pas le temps de parler!");
        return true
    end
...
function handleDockedState()
    ...
    if player:getWeaponStorageMax("Homing") > 0 then
        addCommsReply("Avez-vous des missiles a guidage en stock pour nous? ("..getWeaponCost("Homing").."rep each)", function()
...
function handleWeaponRestock(weapon)
    if not player:isDocked(comms_target) then setCommsMessage("Vous devez rester arrimes pour cette action."); return end
    ...
    if item_amount <= 0 then
        if weapon == "Nuke" then
            setCommsMessage("Toutes les Nanos sont chargees et amorcees en mode annihilation.");
        else
            setCommsMessage("Desole, mais vous etes charges au maximum de ce que je peux vous fournir.");
        end
        addCommsReply("Retour", mainMenu)
    else
        if not player:takeReputationPoints(points_per_item * item_amount) then
            setCommsMessage("Pas assez de trans-crebits.")
            return
        end
        ...
        if player:getWeaponStorage(weapon) == player:getWeaponStorageMax(weapon) then
            setCommsMessage("Vous etes charges au maximum et prets a faire exploser des choses.")
        else
            setCommsMessage("Nous vous avons genereusement fournis avec quelques charges.\nFaites-en bon usage.")
        end
        addCommsReply("Retour", mainMenu)
    end
end
...
function handleUndockedState()
    ...
    if player:isFriendly(comms_target) then
        setCommsMessage("Bonjour, officier.\nSi vous avez besoin de fournitures, arrimez-vous a nous au prealable SVP.")
    else
        setCommsMessage("Bienvenue.\nSi vous voulez negocier, arrimez-vous a nous au prealable SVP.")
    end
    if isAllowedTo(comms_target.comms_data.services.supplydrop) then
        addCommsReply("Pouvez-vous nous envoyer un largage de fournitures? ("..getServiceCost("supplydrop").."rep)", function()
            if player:getWaypointCount() < 1 then
                setCommsMessage("Vous devez deployer une balise avant de demander du support.");
            else
                ...
                for n=1,player:getWaypointCount() do
                    addCommsReply("Balise " .. n, function()
                        if player:takeReputationPoints(getServiceCost("supplydrop")) then
                            ...
                            setCommsMessage("Nous avons depeche une navette de fournitures vers la balise " .. n);
                        else
                            setCommsMessage("Pas assez de trans-crebits!");
                        end
                        addCommsReply("Retour", mainMenu)
    ...
    if isAllowedTo(comms_target.comms_data.services.reinforcements) then
        addCommsReply("Envoyez des renforts SVP! ("..getServiceCost("reinforcements").."rep)", function()
            if player:getWaypointCount() < 1 then
                setCommsMessage("Vous devez deployer une balise avant de demander des renforts.");
            else
                setCommsMessage("Sur quelle balise devons nous deployer les renforts?");
                for n=1,player:getWaypointCount() do
                    addCommsReply("Balise " .. n, function()
                        if player:takeReputationPoints(getServiceCost("reinforcements")) then
                            ...
                            setCommsMessage("Nous avons deploye " .. ship:getCallSign() .. " pour vous aider a la balise " .. n);
                        else
                            setCommsMessage("Pas assez de trans-crebits!");
                        end
                        addCommsReply("Retour", mainMenu)
