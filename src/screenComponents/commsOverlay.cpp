...
: GuiElement(owner, "COMMS_OVERLAY")
{
    ...
    (new GuiLabel(opening_box, "COMMS_OPENING_LABEL", "Ouverture des communications...", 40))->setSize(GuiElement::GuiSizeMax, 50)->setPosition(0, 0, ATopCenter);
    ...
    opening_cancel = new GuiButton(opening_box, "COMMS_OPENING_CANCEL", "Annuler", []()
    {
        if (my_spaceship)
    ...
    hailed_answer = new GuiButton(hailed_box, "COMMS_BEING_HAILED_ANSWER", "Repondre", []() {
        if (my_spaceship)
            my_spaceship->commandAnswerCommHail(true);
    });
    hailed_answer->setSize(300, 50)->setPosition(20, -20, ABottomLeft);

    hailed_ignore = new GuiButton(hailed_box, "COMMS_BEING_HAILED_IGNORE", "Ignorer", []() {
        if (my_spaceship)
    ...
    (new GuiLabel(no_response_box, "COMMS_NO_REPONSE_LABEL", "Pas de reponse...", 40))->setSize(GuiElement::GuiSizeMax, GuiElement::GuiSizeMax)->setPosition(0, 0, ATopLeft);
    ...
    (new GuiLabel(broken_box, "COMMS_BROKEN_LABEL", "Communications interrompues", 40))->setSize(GuiElement::GuiSizeMax, GuiElement::GuiSizeMax)->setPosition(0, 0, ATopLeft);
    ...
    (new GuiLabel(closed_box, "COMMS_BROKEN_LABEL", "Canal de communications ferme", 40))->setSize(GuiElement::GuiSizeMax, GuiElement::GuiSizeMax)->setPosition(0, 0, ATopLeft);
    ...
    chat_comms_close_button = new GuiButton(chat_comms_box, "CLOSE_BUTTON", "Fermer", [this]() {
        if (my_spaceship)
            my_spaceship->commandCloseTextComm();
    });
    ...
    script_comms_close = new GuiButton(script_comms_box, "CLOSE_BUTTON", "Fermer", [this]() {
        script_comms_options->setOptions({});
        if (my_spaceship)
            my_spaceship->commandCloseTextComm();
    });
