...
: GuiElement(owner, id)
{
    ...
    button = new GuiButton(this, id + "_BUTTON", "WARP", [this]() {
        my_spaceship->commandWARP(slider->getValue());
    });
    button->setPosition(0, 0, ABottomLeft)->setSize(GuiElement::GuiSizeMax, 50);
    
    (new GuiPowerDamageIndicator(this, id + "_DPI", SYS_WARPDrive, ATopCenter))->setPosition(0, -50, ABottomLeft)->setSize(50, GuiElement::GuiSizeMax);
}
...
void GuiWARPControls::onDraw(sf::RenderTarget& window)
{
    if (my_spaceship)
    {
        if (my_spaceship->WARP_delay > 0.0)
        {
            label->setKey("WARP in");
            label->setValue(string(int(ceilf(my_spaceship->WARP_delay))));
            slider->disable();
            button->disable();
            charge_bar->hide();
        }else if (my_spaceship->WARP_drive_charge < my_spaceship->WARP_drive_max_distance)
        {
            label->setKey("Charging");
            label->setValue("...");
            slider->hide();
            button->disable();
            charge_bar->setRange(0.0, my_spaceship->WARP_drive_max_distance);
            charge_bar->setValue(my_spaceship->WARP_drive_charge)->show();
        }else{
            label->setKey("Distance");
            label->setValue(string(slider->getValue() / 1000.0, 1) + DISTANCE_UNIT_1K);
            slider->enable()->show();
            slider->setRange(my_spaceship->WARP_drive_max_distance, my_spaceship->WARP_drive_min_distance);
            button->enable();
            charge_bar->hide();
        }
    }
...
void GuiWARPControls::onHotkey(const HotkeyResult& key)
{
    ...
    {
        if (key.hotkey == "INC_WARP")
            slider->setValue(slider->getValue() + 1000.0f);
        if (key.hotkey == "DEC_WARP")
            slider->setValue(slider->getValue() - 1000.0f);
        if (key.hotkey == "WARP")
            my_spaceship->commandWARP(slider->getValue());
    }
