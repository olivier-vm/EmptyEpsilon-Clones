...
#include "spaceObjects/RLSJammer.h"
...
bool HardwareController::getVariableValue(string variable_name, float& value)
{
    ...
    SHIP_VARIABLE("RLS", ship->current_RLS * ship->getSystemEffectiveness(SYS_RLS));
    ...
    SHIP_VARIABLE("IsJammed", RLSJammer::isRLSJammed(ship->getPosition()) ? 1.0f : 0.0f);
    SHIP_VARIABLE("WARPing", ship->WARP_delay > 0.0f ? 1.0f : 0.0f);
    SHIP_VARIABLE("WARPed", ship->WARP_indicator > 0.0f ? 1.0f : 0.0f);
