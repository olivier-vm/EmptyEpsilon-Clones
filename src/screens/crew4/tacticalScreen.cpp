...
: GuiOverlay(owner, "TACTICAL_SCREEN", colorConfig.background)
{
    ...
    // LASER controls beneath the radar.
    if (gameGlobalInfo->use_LASER_shield_frequencies || gameGlobalInfo->use_system_damage)
    {
        ...
        LASER_info_box->setPosition(0, -20, ABottomCenter)->setSize(500, 50);
        (new GuiLabel(LASER_info_box, "LASER_INFO_LABEL", "LASERs", 30))->addBackground()->setPosition(0, 0, ABottomLeft)->setSize(80, 50);
        (new GuiLASERFrequencySelector(LASER_info_box, "LASER_FREQUENCY_SELECTOR"))->setPosition(80, 0, ABottomLeft)->setSize(132, 50);
        (new GuiPowerDamageIndicator(LASER_info_box, "", SYS_LASERWeapons, ACenterLeft))->setPosition(0, 0, ABottomLeft)->setSize(212, 50);
        (new GuiLASERTargetSelector(LASER_info_box, "LASER_TARGET_SELECTOR"))->setPosition(0, 0, ABottomRight)->setSize(288, 50);
    }
