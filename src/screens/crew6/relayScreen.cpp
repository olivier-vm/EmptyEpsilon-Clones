...
: GuiOverlay(owner, "RELAY_SCREEN", colorConfig.background), mode(TargetSelection)
{
    ...
    info_callsign = new GuiKeyValueDisplay(sidebar, "SCIENCE_CALLSIGN", 0.4, "Indicatif", "");
    info_callsign->setSize(GuiElement::GuiSizeMax, 30);

    info_faction = new GuiKeyValueDisplay(sidebar, "SCIENCE_FACTION", 0.4, "Patrie", "");
    ...
    // Open comms button.
    if (allow_comms == true)
        (new GuiOpenCommsButton(option_buttons, "OPEN_COMMS_BUTTON", "Ouvrir Communications", &targets))->setSize(GuiElement::GuiSizeMax, 50);
    else
        (new GuiOpenCommsButton(option_buttons, "OPEN_COMMS_BUTTON", "Lier aux Communications", &targets))->setSize(GuiElement::GuiSizeMax, 50);


    // Hack target
    hack_target_button = new GuiButton(option_buttons, "HACK_TARGET", "Demarrer piratage", [this](){
    ...
    link_to_science_button = new GuiToggleButton(option_buttons, "LINK_TO_SCIENCE", "Lien vers Science", [this](bool value){
        if (value)
    ...
    // Alert level buttons.
    alert_level_button = new GuiToggleButton(layout, "", "Niveau d'alerte", [this](bool value)
    {
