...
: GuiOverlay(owner, "ENGINEERING_SCREEN", colorConfig.background), selected_system(SYS_None)
{
    ...
    hull_display = new GuiKeyValueDisplay(this, "HULL_DISPLAY", 0.45, "Coque", "");
    ...
    total_coolant_label = new GuiLabel(box, "TOTAL_COOLANT_LABEL", "Refrigerant Total", 30);
...
void EngineeringScreen::onDraw(sf::RenderTarget& window)
{
    if (my_spaceship)
    {
        ...
        total_coolant_label->setText("Refrigerant Total: " + string(int(total_coolant / PlayerSpaceship::max_coolant_per_system * 100)) + "/" + string(int(my_spaceship->max_coolant / PlayerSpaceship::max_coolant_per_system * 100)));
        ...
        if (selected_system != SYS_None)
        {
            ...
            coolant_label->setText("Refrigerant: " + string(int(system.coolant_level / PlayerSpaceship::max_coolant_per_system * 100)) + "/" + string(int(std::min(system.coolant_request, my_spaceship->max_coolant) / PlayerSpaceship::max_coolant_per_system * 100)));
            ...
            case SYS_LASERWeapons:
                addSystemEffect("Cadence de tir", string(int(effectiveness * 100)) + "%");
                ...
                for(int n = 0; n < max_LASER_weapons; n++)
                {
                    ...
                    {
                        addSystemEffect("Vitesse de rotation de tourelle", string(int(effectiveness * 100)) + "%");
                        break;
                    }
                }
                break;
            ...
            case SYS_RLS:
                ...
                break;
            case SYS_WARPDrive:
                addSystemEffect("Taux de recharge du moteur WARP", string(int(my_spaceship->getWARPDriveRechargeRate() * 100)) + "%");
                addSystemEffect("Vitesse WARP", string(int(effectiveness * 100)) + "%");
                break;
            case SYS_FrontShield:
                if (gameGlobalInfo->use_LASER_shield_frequencies)
                    addSystemEffect("Vitesse de calibration", string(int((my_spaceship->getSystemEffectiveness(SYS_FrontShield) + my_spaceship->getSystemEffectiveness(SYS_RearShield)) / 2.0 * 100)) + "%");
                addSystemEffect("Taux de charge", string(int(effectiveness * 100)) + "%");
                {
                    DamageInfo di;
                    di.type = DT_Kinetic;
                    float damage_negate = 1.0f - my_spaceship->getShieldDamageFactor(di, 0);
                    if (damage_negate < 0.0)
                        addSystemEffect("Dommage extra", string(int(-damage_negate * 100)) + "%");
                    else
                        addSystemEffect("Dommage neutralise", string(int(damage_negate * 100)) + "%");
                }
                break;
            case SYS_RearShield:
                if (gameGlobalInfo->use_LASER_shield_frequencies)
