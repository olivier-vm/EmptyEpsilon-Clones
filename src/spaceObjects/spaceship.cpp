...
REGISTER_SCRIPT_SUBCLASS_NO_CREATE(SpaceShip, ShipTemplateBasedObject)
{
    ...
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERFrequency);
    ...
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setWARPDriveRange);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, hasRLSDrive);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setRLSDrive);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponArc);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponDirection);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponRange);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponTurretArc);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponTurretDirection);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponCycleTime);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponDamage);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponEnergyPerFire);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, getLASERWeaponHeatPerFire);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setLASERWeapon);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setLASERWeaponTurret);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setLASERWeaponTexture);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setLASERWeaponEnergyPerFire);
    REGISTER_SCRIPT_CLASS_FUNCTION(SpaceShip, setLASERWeaponHeatPerFire);
...
: ShipTemplateBasedObject(50, multiplayerClassName, multiplayer_significant_range)
{
    ...
    WARP_drive_charge = WARP_drive_max_distance;
    WARP_distance = 0.0;
    WARP_delay = 0.0;
    wormhole_alpha = 0.0;
    weapon_tube_count = 0;
    ...
    RLS_speed_per_RLS_level = 1000.0;
    ...
    target_id = -1;
    LASER_frequency = irandom(0, max_frequency);
    ...
    registerMemberReplication(&has_RLS_drive);
    registerMemberReplication(&RLS_request, 0.1);
    registerMemberReplication(&current_RLS, 0.1);
    registerMemberReplication(&has_WARP_drive);
    registerMemberReplication(&WARP_drive_charge, 0.5);
    registerMemberReplication(&WARP_delay, 0.5);
    registerMemberReplication(&WARP_drive_min_distance);
    registerMemberReplication(&WARP_drive_max_distance);
    ...
    registerMemberReplication(&RLS_speed_per_RLS_level);
    registerMemberReplication(&shield_frequency);
    registerMemberReplication(&docking_state);
    registerMemberReplication(&LASER_frequency);
    ...
    {
        LASER_weapons[n].setParent(this);
    }
...
void SpaceShip::applyTemplateValues()
{
    for(int n=0; n<max_LASER_weapons; n++)
    {
        LASER_weapons[n].setPosition(ship_template->model_data->getLASERPosition(n));
        LASER_weapons[n].setArc(ship_template->LASERs[n].getArc());
        LASER_weapons[n].setDirection(ship_template->LASERs[n].getDirection());
        LASER_weapons[n].setRange(ship_template->LASERs[n].getRange());
        LASER_weapons[n].setTurretArc(ship_template->LASERs[n].getTurretArc());
        LASER_weapons[n].setTurretDirection(ship_template->LASERs[n].getTurretDirection());
        LASER_weapons[n].setTurretRotationRate(ship_template->LASERs[n].getTurretRotationRate());
        LASER_weapons[n].setCycleTime(ship_template->LASERs[n].getCycleTime());
        LASER_weapons[n].setDamage(ship_template->LASERs[n].getDamage());
        LASER_weapons[n].setLASERTexture(ship_template->LASERs[n].getLASERTexture());
        LASER_weapons[n].setEnergyPerFire(ship_template->LASERs[n].getEnergyPerFire());
        LASER_weapons[n].setHeatPerFire(ship_template->LASERs[n].getHeatPerFire());
    }
    ...
    has_RLS_drive = ship_template->RLS_speed > 0.0;
    RLS_speed_per_RLS_level = ship_template->RLS_speed;
    has_WARP_drive = ship_template->has_WARP_drive;
    WARP_drive_min_distance = ship_template->WARP_drive_min_distance;
    WARP_drive_max_distance = ship_template->WARP_drive_max_distance;
    for(int n=0; n<max_weapon_tubes; n++)
    {
...
void SpaceShip::draw3DTransparent()
{
    ...
    if ((has_WARP_drive && WARP_delay > 0.0f) ||
        (wormhole_alpha > 0.0f))
    {
        float delay = WARP_delay;
...
{
    // Draw LASER arcs on short-range radar only, and only for fully scanned
    // ships.
    if (!long_range && (!my_spaceship || (getScannedStateFor(my_spaceship) == SS_FullScan)))
    {
        ...
        for(int n = 0; n < max_LASER_weapons; n++)
        {
            // Draw LASER arcs only if the LASER has a range. A LASER with range 0
            // effectively doesn't exist; exit if that's the case.
            if (LASER_weapons[n].getRange() == 0.0) continue;

            // Color LASER arcs red.
            // TODO: Make this color configurable.
            sf::Color color = sf::Color::Red;

            // If the LASER is cooling down, flash and fade the arc color.
            if (LASER_weapons[n].getCooldown() > 0)
                color = sf::Color(255, 255 * (LASER_weapons[n].getCooldown() / LASER_weapons[n].getCycleTime()), 0);

            // Initialize variables from the LASER's data.
            float LASER_direction = LASER_weapons[n].getDirection();
            float LASER_arc = LASER_weapons[n].getArc();
            float LASER_range = LASER_weapons[n].getRange();

            // Set the LASER's origin on radar to its relative position on the
            // mesh.
            sf::Vector2f LASER_offset = sf::rotateVector(ship_template->model_data->getLASERPosition2D(n) * scale, getRotation());
            ...
            // Drop the pen onto the LASER's origin.
            a[0].position = LASER_offset + position;

            // Draw the LASER's left bound.
            a[1].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction + LASER_arc / 2.0f)) * LASER_range * scale;
            a[2].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction + LASER_arc / 2.0f)) * LASER_range * scale * 1.3f;
            window.draw(a);

            // Draw the LASER's right bound.
            a[1].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction - LASER_arc / 2.0f)) * LASER_range * scale;
            a[2].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction - LASER_arc / 2.0f)) * LASER_range * scale * 1.3f;
            window.draw(a);

            // Draw the LASER's arc.
            int arcPoints = int(LASER_arc / 10) + 1;
            sf::VertexArray arc_line(sf::LinesStrip, arcPoints);
            for(int i=0; i<arcPoints; i++)
            {
                arc_line[i].color = color;
                arc_line[i].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction - LASER_arc / 2.0f + 10 * i)) * LASER_range * scale;
            }
            arc_line[arcPoints-1].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (LASER_direction + LASER_arc / 2.0f)) * LASER_range * scale;
            window.draw(arc_line);

            // If the LASER is turreted, draw the turret's arc. Otherwise, exit.
            if (LASER_weapons[n].getTurretArc() == 0.0) continue;

            // Initialize variables from the turret data.
            float turret_arc = LASER_weapons[n].getTurretArc();
            float turret_direction = LASER_weapons[n].getTurretDirection();

            // Draw the turret's bounds, at half the transparency of the LASER's.
            // TODO: Make this color configurable.
            a[0].color = sf::Color(color.r, color.g, color.b, color.a / 2);
            a[1].color = sf::Color(color.r, color.g, color.b, color.a / 2);

            // Draw the turret's left bound. (We're reusing the LASER's origin.)
            a[1].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (turret_direction + turret_arc / 2.0f)) * LASER_range * scale;
            a[2].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (turret_direction + turret_arc / 2.0f)) * LASER_range * scale * 1.3f;
            window.draw(a);
            ...
            for(int i = 0; i < turret_points; i++)
            {
                ...
                turret_line[i].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (turret_direction - turret_arc / 2.0f + 10 * i)) * LASER_range * scale;
            }
            turret_line[turret_points-1].position = LASER_offset + position + sf::vector2FromAngle(getRotation() + (turret_direction + turret_arc / 2.0f)) * LASER_range * scale;
...
void SpaceShip::update(float delta)
{
    ...
    if ((has_WARP_drive && WARP_delay > 0) || (has_RLS_drive && RLS_request > 0))
    {
        if (RLSJammer::isRLSJammed(getPosition()))
        {
            WARP_delay = 0;
            RLS_request = 0.0f;
        }
    }
    if (has_WARP_drive && WARP_delay > 0)
    {
        ...
        WARP_delay -= delta * getSystemEffectiveness(SYS_WARPDrive);
        if (WARP_delay <= 0.0)
        {
            executeWARP(WARP_distance);
            WARP_delay = 0.0;
        }
    }else if (has_RLS_drive && (RLS_request > 0 || current_RLS > 0))
    {
        if (current_impulse > 0.0)
        {
            if (impulse_max_speed > 0)
            ...
            if (current_RLS < RLS_request)
            {
                current_RLS += delta / RLS_charge_time;
                if (current_RLS > RLS_request)
                    current_RLS = RLS_request;
            }else if (current_RLS > RLS_request)
            {
                current_RLS -= delta / RLS_decharge_time;
                if (current_RLS < RLS_request)
                    current_RLS = RLS_request;
            }
        }
        ...
        if (has_WARP_drive)
        {
            float f = getWARPDriveRechargeRate();
            if (f > 0)
            {
                if (WARP_drive_charge < WARP_drive_max_distance)
                {
                    float extra_charge = (delta / WARP_drive_charge_time * WARP_drive_max_distance) * f;
                    if (useEnergy(extra_charge * WARP_drive_energy_per_km_charge / 1000.0))
                    {
                        WARP_drive_charge += extra_charge;
                        if (WARP_drive_charge >= WARP_drive_max_distance)
                            WARP_drive_charge = WARP_drive_max_distance;
                    }
                }
            }else{
                WARP_drive_charge += (delta / WARP_drive_charge_time * WARP_drive_max_distance) * f;
                if (WARP_drive_charge < 0.0f)
    ...
    // Add heat based on RLS factor.
    addHeat(SYS_RLS, current_RLS * delta * heat_per_RLS);

    // Determine forward direction and velocity.
    sf::Vector2f forward = sf::vector2FromAngle(getRotation());
    setVelocity(forward * (current_impulse * impulse_max_speed * getSystemEffectiveness(SYS_Impulse) + current_RLS * RLS_speed_per_RLS_level * getSystemEffectiveness(SYS_RLS)));
    ...
    if (has_WARP_drive && WARP_delay > 0.0f)
        model_info.RLS_scale = (10.0f - WARP_delay) / 10.0f;
    else
        model_info.RLS_scale = 0.0;
}
...
void SpaceShip::executeWARP(float distance)
{
    float f = systems[SYS_WARPDrive].health;
    if (f <= 0.0)
        return;
    ...
    if (RLSJammer::isRLSJammed(target_position))
        target_position = RLSJammer::getFirstNoneJammedPosition(getPosition(), target_position);
    setPosition(target_position);
    addHeat(SYS_WARPDrive, WARP_drive_heat_per_WARP);
}
...
void SpaceShip::initializeWARP(float distance)
{
    if (docking_state != DS_NotDocking)
        return;
    if (WARP_drive_charge < WARP_drive_max_distance) // You can only WARP when the drive is fully charged
        return;
    if (WARP_delay <= 0.0)
    {
        WARP_distance = distance;
        WARP_delay = 10.0;
        WARP_drive_charge -= distance;
    }
}
...
float SpaceShip::getShieldDamageFactor(DamageInfo& info, int shield_index)
{
    ...
    if (info.type == DT_Energy && gameGlobalInfo->use_LASER_shield_frequencies)
    {
    ...
    if (gameGlobalInfo->use_system_damage)
    {
        if (info.system_target != SYS_None)
        {
            ...
            if (info.type == DT_Energy)
                system_damage *= 3.0;   //LASER weapons do more system damage, as they penetrate the hull easier.
            ...
            if (info.type == DT_Energy)
                system_damage *= 2.5;   //LASER weapons do more system damage, as they penetrate the hull easier.
            systems[random_system].health -= system_damage;
            if (systems[random_system].health < -1.0)
                systems[random_system].health = -1.0;
        }
...
string SpaceShip::getScriptExportModificationsOnTemplate()
{
    ...
    if (has_WARP_drive != ship_template->has_WARP_drive)
        ret += ":setWARPDrive(" + string(has_WARP_drive ? "true" : "false") + ")";
    if (has_RLS_drive != (ship_template->RLS_speed > 0))
        ret += ":setRLSDrive(" + string(has_RLS_drive ? "true" : "false") + ")";
    ...
    for(int n=0; n<max_LASER_weapons; n++)
    {
        if (LASER_weapons[n].getArc() != ship_template->LASERs[n].getArc()
         || LASER_weapons[n].getDirection() != ship_template->LASERs[n].getDirection()
         || LASER_weapons[n].getRange() != ship_template->LASERs[n].getRange()
         || LASER_weapons[n].getTurretArc() != ship_template->LASERs[n].getTurretArc()
         || LASER_weapons[n].getTurretDirection() != ship_template->LASERs[n].getTurretDirection()
         || LASER_weapons[n].getTurretRotationRate() != ship_template->LASERs[n].getTurretRotationRate()
         || LASER_weapons[n].getCycleTime() != ship_template->LASERs[n].getCycleTime()
         || LASER_weapons[n].getDamage() != ship_template->LASERs[n].getDamage())
        {
            ret += ":setLASERWeapon(" + string(n) + ", " + string(LASER_weapons[n].getArc(), 0) + ", " + string(LASER_weapons[n].getDirection(), 0) + ", " + string(LASER_weapons[n].getRange(), 0) + ", " + string(LASER_weapons[n].getCycleTime(), 1) + ", " + string(LASER_weapons[n].getDamage(), 1) + ")";
            ret += ":setLASERWeaponTurret(" + string(n) + ", " + string(LASER_weapons[n].getTurretArc(), 0) + ", " + string(LASER_weapons[n].getTurretDirection(), 0) + ", " + string(LASER_weapons[n].getTurretRotationRate(), 0) + ")";
        }
    }
...
{
    if (LASER_frequency < 0 || shield_frequency < 0)
        return 1.0;

    float diff = abs(LASER_frequency - shield_frequency);
